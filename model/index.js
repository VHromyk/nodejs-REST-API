...
const getContactById = async (contactId) => {
  try {
    ...
    const findContactById = response.find(el => el.id.toString() === contactId);
...
const removeContact = async (contactId) => {
    try {
      const response = await listContacts();
      const filterElements = response.filter(el => el.id.toString() !== contactId);
...
const updateContact = async (contactId, body) => {
    ...
    const response = await listContacts();
      ...
      const arrWithoutUpdateContact = response.filter(el => el.id.toString() !== contactId)
